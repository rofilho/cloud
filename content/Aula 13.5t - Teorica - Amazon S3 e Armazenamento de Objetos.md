...
**Enunciado:** Uma multinacional de varejo brasileira utiliza um data lake que processa mais de 100 TB de dados semanais gerados por compras online. Os relatórios de auditoria desses dados são extremamente acessados nas primeiras duas semanas após a geração. Após esse período, o acesso despenca para menos de uma consulta mensal de auditoria técnica. No entanto, por conformidade legal regulamentar nacional, todos esses dados brutos de relatórios devem ser guardados intactos pelo prazo de 7 anos. Qual arquitetura de ciclo de vida do Amazon S3 é a mais eficiente em termos de custos (FinOps) e atende integralmente à regulação?

- [ ] A) Manter todos os dados na classe S3 Standard, aplicando criptografia de ponta e auditoria ativada no CloudTrail durante os 7 anos.
- [ ] B) Iniciar o upload em S3 Standard-IA, migrar para S3 One Zone-IA após 15 dias e realizar a exclusão automática via Lifecycle após 7 anos.
- [x] C) Iniciar o upload na classe S3 Standard, criar uma regra de ciclo de vida para transicionar os dados para S3 Standard-IA após 30 dias, transicionar para S3 Glacier Deep Archive após 90 dias, e expirar/deletar definitivamente os dados após 2555 dias (7 anos). ✅
- [ ] D) Provisionar um disco permanente EBS (gp3) anexado a uma instância EC2 dedicada 24/7 com rotinas de script CRON locais para compactar logs em zip mensalmente.

**Justificativa:** A classe S3 Standard atende ao tráfego pesado inicial. O Standard-IA é perfeito para o período de acesso esporádico subsequente. O Glacier Deep Archive oferece a tarifa de armazenamento mais barata da nuvem para os anos restantes de conformidade regulamentar, onde o acesso é extremamente raro. A expiração aos 7 anos (2555 dias) encerra o ciclo de vida de forma automatizada sem desperdício de fundos ou intervenção humana manual.

---

### Questão 2 (Múltipla Escolha — Nível: Intermediário)
**Enunciado:** Um engenheiro DevOps está implementando uma API REST em Python utilizando o framework Flask dentro de uma instância Amazon EC2. Esta API deve ler arquivos do S3 em tempo de execução. Para obter o acesso, o programador novato gerou uma chave de acesso programática (`Access Key` e `Secret Key`) na sua conta pessoal do console AWS e colou estas credenciais como variáveis fixas dentro do arquivo de código fonte `app.py` da aplicação. Analise a postura de segurança dessa arquitetura e selecione a correção mais recomendada pelas diretrizes de Well-Architected Framework da AWS:

- [ ] A) A arquitetura está correta, mas as credenciais devem ser salvas criptografadas com chave SSH RSA de 4096 bits no arquivo `/etc/environment` do Linux.
- [ ] B) O S3 Bucket deve ter a Bucket Policy atualizada para pública, eliminando a necessidade de qualquer senha no backend.
- [x] C) As chaves estáticas devem ser removidas do código fonte. Deve-se criar uma IAM Role com a política de acesso de leitura (S3ReadOnly) e anexar essa Role diretamente ao perfil da instância EC2. O SDK (Boto3) herda as credenciais dinâmicas do metadado da máquina automaticamente. ✅
- [ ] D) Migrar o backend da EC2 para um portal estático diretamente no S3 Bucket, eliminando totalmente a camada dinâmica de execução computacional.

**Justificativa:** Credenciais estáticas em código geram altíssimo risco de vazamento (ex: commits acidentais no GitHub público). O mecanismo recomendado pela AWS é o uso de IAM Roles anexadas à EC2. A instância assume o perfil temporário de forma transparente e segura, eliminando chaves permanentes expostas no ambiente.

---

### Questão 3 (Dissertativa — Nível: Avançado)
**Enunciado:** Startups de tecnologia e equipes de SRE comumente optam por arquiteturas de frontend desacopladas, hospedando seus sites corporativos e aplicações dinâmicas de frontend (Single Page Applications em React/Vue/Angular) em buckets do Amazon S3 com a funcionalidade de *Static Website Hosting* ativa, em vez de implantarem servidores virtuais (Nginx/Apache em instâncias EC2). 

Com base nos princípios de arquitetura Cloud, responda aos itens abaixo:
1. Explique os benefícios operacionais, financeiros e de escalabilidade associados a essa decisão.
2. Descreva o comportamento de concorrência de origem (**CORS**) que comumente ocorre quando esse frontend estático tenta buscar dados dinâmicos de uma API rodando em uma EC2 na mesma região e apresente a solução técnica necessária no backend para resolver o problema.

**Resposta esperada:**
- **Benefícios (Parte 1):** Operacionalmente, elimina a gerência de servidores (patches de S.O., manutenção de runtime, patches de segurança), reduzindo a carga do time de sysadmins. Financeiramente, remove o custo fixo de instâncias ligadas 24/7, cobrando apenas o armazenamento consumido e as requisições HTTP realizadas (centavos por milhão de acessos). Em termos de escalabilidade, o S3 absorve picos massivos de tráfego de forma nativa e automática, sem necessidade de configurar balanceadores de carga ou políticas de Auto Scaling.
- **CORS e Solução (Parte 2):** O bloqueio de CORS ocorre no navegador do usuário. Como o frontend estático é servido por um domínio (S3 Website Endpoint) e tenta se comunicar assincronamente (fetch/AJAX) com outro domínio/IP (API na EC2), o navegador bloqueia a transação por restrição de segurança do mesmo domínio. Para solucionar, o desenvolvedor deve configurar cabeçalhos CORS no servidor de backend da EC2 (ex: injetar headers `Access-Control-Allow-Origin` contendo a URL exata do S3 ou o curinga `*` na resposta HTTP) indicando que a transação cruzada é legítima e autorizada pelo servidor.
