...
**Regra de Ouro (Infraestrutura):** Todo arquivo que começa com `_` (underline) é estritamente privado do professor e será banido pelo gerador do site. Sempre configure a nova disciplina no `quartz.config.ts` para ignorar o padrão `_*.md`.
